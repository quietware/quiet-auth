...
/**
 * Persists the user's 2FA list under [STORAGE_KEY] using the shared JSON layout from the
 * reference token repository adapter (TypeScript), so data shape stays portable across clients.
 */
class TokenRepository(private val storage: SecureStorage) {
    ...
    companion object {
        const val STORAGE_KEY = "quietauth_twofa_items_v1"
    }
}
